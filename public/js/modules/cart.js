...
export function actualizarTabla() {
	...
	carrito.forEach((item, index) => {
		...
		tr.innerHTML = `
			<td data-label="Código" class="p-4 font-bold text-slate-600">${codigoMostrado}</td>
			<td data-label="Descripción" class="p-4 text-slate-500">
				<div>${escapeHtml(item.descripcion)}</div>
				${marcaLabel}
			</td>
			<td data-label="Cant." class="p-4 text-center font-bold">${qtyCell}</td>
			<td data-label="P. unit" class="p-4 text-right text-slate-400 font-mono cursor-help" data-role="precio-usd" data-tooltip="$${formatNumber(item.precio_usd, 2)} • ≈ ${formatNumber(precioBs, 2)} Bs">$${formatNumber(item.precio_usd, 2)}</td>
			<td data-label="Subtotal" class="p-4 text-right font-black ${modoDevolucion ? 'text-rose-600' : 'text-blue-600'} font-mono cursor-help" data-role="subtotal-usd" data-tooltip="$${formatNumber(subtotalUSD, 2)} • ≈ ${formatNumber(subtotalBs, 2)} Bs">$${formatNumber(subtotalUSD, 2)}</td>
